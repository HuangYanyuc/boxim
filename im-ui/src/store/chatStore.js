        ...
        checkChat(state, id) {
            if (id.source === 'chat') {
                ...
                state.chats.forEach(i => {
                    if (i.targetId === id.targetId && i.type === id.type) {
                        v = i
                    }
                })
                if (Object.keys(v).length > 0) {
                    ...
                    this.commit('openChat', id)
                }
            } else if (id.source === 'message') {
                let v = {}
                state.chats.forEach(i => {
                    if (i.targetId === id.belonging.targetId && i.type === id.belonging.type) {
                        v = i
                    }
                })
                if (Object.keys(v).length > 0) {
                    state.activeChat = v
                    let tem = 'msg' + id.id + id.sendId
                    this.commit('setMsgLocalization', tem)
                } else {
                    this.commit('openChat', id)
                }
            }
