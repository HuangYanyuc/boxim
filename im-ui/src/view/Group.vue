		...
		<el-aside width="260px" class="group-list-box">
			...
			<el-scrollbar class="group-list-items">
				<div v-for="(group, index) in groupStore.groups" :key="index">
					<group-item v-show="!group.quit && groupItemShowFn(group)" :group="group"
				...
				<div v-show="activeGroup.id">
					<div class="group-info">
						<div>
							...
							<el-button class="send-btn" icon="el-icon-chat-dot-round" type="primary"
								@click="onSendMessage()">发送消息</el-button>
						</div>
...
export default {
	...
	methods: {
		...
		groupItemShowFn(group) {
			...
			ifShow = ifShow || group?.name.indexOf(this.searchText) != '-1'
			return ifShow
		}
	},
	computed: {
